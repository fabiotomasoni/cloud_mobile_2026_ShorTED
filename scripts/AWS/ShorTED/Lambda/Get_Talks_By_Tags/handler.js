...
module.exports.get_by_tags = async (event, context, callback) => {
    ...
    try {
        ...
        const matchedSnacks = await snack.find({ tags: { $in: tags } })
            .select('_id segmentId talkId talkSlug speaker talkTitle topic quote motivationalText aphorism tags score startTime endTime talkUrl embedUrl thumbnailUrl thumbnailUrlHd thumbnailUrlFullHd hlsUrl mp4Url mediaExtractedAt mediaExtractionVersion mediaExtractionStatus mediaExtractionError language aiPipelineVersion sourceHash createdAt')
