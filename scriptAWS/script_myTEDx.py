...
#MODIFIED SCRIPT FOR "MYTEDX"-HOMEWORK 2 - ADDED FUNCTIONALITY TO CREATE AN AGGREGATE MODEL WITH RELATED VIDEOS
...
##### FROM FILES
tedx_dataset_path = "s3://tedx-2026-data-fgt/final_list.csv"
...
## READ THE DETAILS
details_dataset_path = "s3://tedx-2026-data-fgt/details.csv"
...
## READ TAGS DATASET
tags_dataset_path = "s3://tedx-2026-data-fgt/tags.csv"
...
related_videos_dataset_path = "s3://tedx-2026-data-fgt/related_videos.csv"
...
related_videos_dataset_agg = related_videos_dataset.groupBy(col("id").alias("id_ref")).agg(
    collect_list(
        struct(
            col("related_id"),
            col("slug"),
...
tedx_dataset_agg = tedx_dataset_main.join(tags_dataset_agg, tedx_dataset.id == tags_dataset_agg.id_ref, "left") \
    .drop("id_ref") \
    .join(related_videos_dataset_agg, tedx_dataset.id == related_videos_dataset_agg.id_ref, "left") \
    .drop("id_ref") \
